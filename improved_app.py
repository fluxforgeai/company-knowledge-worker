...
def parse_arguments():
    ...
    parser.add_argument(
        "--use-improved-rag",
        action="store_true",
        help="Use improved RAG pipeline with better project coverage"
    )
    
    return parser.parse_args()

def initialize_system(rebuild_db: bool = False, use_improved_rag: bool = False):
    """Initialize the complete system"""
    logger = get_logger(__name__)
    
    try:
        logger.info("🔄 Initializing Enhanced Company Knowledge Worker system...")
        ...
        # Initialize RAG pipeline (standard or improved)
        if use_improved_rag:
            logger.info("🚀 Initializing IMPROVED RAG pipeline...")
            rag_pipeline = ImprovedRAGPipeline(vector_store_manager)
        else:
        ...
        if test_result["success"]:
            ...
            if use_improved_rag:
                logger.info("🎯 Using IMPROVED RAG with enhanced project coverage!")
            return rag_pipeline
        else:
...
def run_web_interface(rag_pipeline, port: int = None, share: bool = False, open_browser: bool = True):
...
def run_cli_interface(rag_pipeline):
    """Run the command line interface"""
    logger = get_logger(__name__)
    
    try:
        logger.info("💻 Starting enhanced command line interface...")
...
# Optional: Retrieval configuration (enhanced default)
RETRIEVAL_K=25
...
def main():
    ...
    try:
        ...
        if args.mode == "build":
            build_vector_store()
            logger.info("✅ Enhanced vector store build completed!")
            return 0
        
        # Initialize system
        rag_pipeline = initialize_system(
            rebuild_db=args.rebuild_db, 
            use_improved_rag=args.use_improved_rag
        )
